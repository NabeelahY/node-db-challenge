...
- Relational databases are applications/systems that store data in a tabular form while SQL is a language we can use to query a database.  

1. Why do tables need a `primary key`?
- Primary keys are needed to unique differenciate and identify the data in that particular table.

1. What is the name given to a table column that references the primary key on another table.
- Foriegn key

1. What do we need in order to have a _many to many_ relationship between two tables.
- An intermidiary table - table that contains foreign keys
